...
export type ContextPeriod = {kind:"today"|"yesterday"|"week"|"explicit";label:string;from:string;to:string;timezone:string};
...
export function resolveContextPeriod(query: string, options: ContextOptions = {}): ContextPeriod | null {
  ...
  const kind = /\bontem\b/.test(normalized) ? "yesterday" : /\b(esta|essa|nesta|nessa) semana\b/.test(normalized) ? "week"
  ...
  if (kind === "week") local.setUTCDate(local.getUTCDate() - (local.getUTCDay()+6)%7);
  const start = local.toISOString().slice(0,10);
  local.setUTCDate(local.getUTCDate() + (kind === "week" ? 7 : 1));
  const end = local.toISOString().slice(0,10);
  return {kind,label:`${kind === "today" ? "hoje" : kind === "yesterday" ? "ontem" : "esta semana"} (${start})`,
...
export function contextSearchTerms(query:string):string {
  const filler = new Set(["como","posso","pode","poderia","quero","preciso","ajude","ajuda","ajudar","meu","meus","minha","minhas","hoje","ontem","esta","essa","nesta","nessa","semana","dia","coach","voce","para","sobre","com","foi","estou","fazer","revisar","revise","avaliar","avalie"]);
