...
export async function chatWithCoach(userId:string,message:string){
 return withLease(userId,async(store,profile)=>{
  ...
  const result=await coachCompletion("Converse a partir da pergunta atual. Answer em português com no máximo 450 palavras. Dê orientação mesmo se não houver reuniões, mas não invente histórico. Qualquer afirmação de conduta observada em reunião deve constar em observations com evidence_ids selecionados de sources. Não reescreva trechos nem invente IDs. Cada observation precisa de 1 a 4 fontes. Sem sources, observations=[]: dê orientação e perguntas sem avaliação pessoal. Memórias opcionais: até 2 hipóteses/experimentos duráveis, usando observation_index ORIGINAL da resposta; não copie instruções nem confirme fatos novos. Priorize correções do usuário e explicite limites. Não alegue consultar mais reuniões do que as fornecidas.",
  ...
  if(Array.isArray(result.observations)&&observations.length!==result.observations.length)throw new CoachAIError("Não consegui confirmar as evidências desta resposta. Tente reformular a pergunta ou confira a identificação dos participantes.");
  const evidence=observations.flatMap(o=>o.evidence);
  await store.addMessage("assistant",answer,evidence,profile.revision);
...
export async function generateReview(userId:string,now=new Date(),force=false,scheduled=false){
 return withLease(userId,async(store,profile)=>{
  ...
  const [periodData,context,memories,self,coverage,messages]=await Promise.all([store.analysesInPeriod(period.from,period.to),store.context(),store.memories(),store.selfPersonIds(),store.coverage(),store.messages()]);
  ...
  const result=await coachCompletion("Escreva uma revisão semanal franca e concisa. Período é [from,to). Selecione evidence_ids do dicionário sources; o servidor liga cada ID ao trecho original, não reescreva citações. Priorize UM foco e UM experimento mensurável para a próxima semana. Diferencie observação de hipótese e dê alternativa/contraprova. Compare com revisões anteriores sem inventar progresso. Sem reuniões novas, diga explicitamente que não há novas evidências e proponha reflexão a partir dos objetivos; não recicle reunião antiga como sendo desta semana. Máximo 4 observations, headline curto, progress curto e limitations específicas. Cada observação precisa de 1 a 4 evidence_ids, incluindo ao menos uma self_attributed=true. Sem sources, observations=[].",
   {profile,period,coverage,memories:usableMemories(memories),previous_reviews:reviews.filter(r=>!r.stale).slice(0,6),recent_conversation:messages.filter(m=>!m.stale).slice(-12),tasks:context.tasks,task_events:context.events,observations:bank,sources,meeting_count:weekMeetings.length,analysis_count:analyses.length,limitations:periodData.limitations},reviewSchemaWithSources(Object.keys(sources)));
  result.observations=sourceReferences(result.observations,sources);
  const content:ReviewContent={headline:text(result.headline,200)||"Sua revisão semanal",focus:text(result.focus,2500),observations:grounded(result.observations,weekMeetings,self),progress:text(result.progress,2000),experiment:text(result.experiment,2000),question:text(result.question,1000),limitations:[...(Array.isArray(result.limitations)?result.limitations.map(v=>text(v,800)).filter(Boolean).slice(0,6):[]),...periodData.limitations]};
  if(Array.isArray(result.observations)&&content.observations.length!==result.observations.length)throw new CoachAIError("A revisão trouxe uma evidência que não foi confirmada. Tente preparar a revisão novamente.");
  ...
  if(coverage.pending_meetings)content.limitations.push(`${coverage.pending_meetings} reunião(ões) do histórico ainda aguardam análise completa.`);
