...
const concreteStep=/^(?:eu )?(?:vou |me comprometo a |decidi )(?:enviar|concluir|terminar|revisar|confirmar|ligar|escrever|preparar|entregar|validar|testar|reservar|organizar|escolher|definir|conversar com|falar com|delegar|pausar|limitar|registrar|cobrar|finalizar)\s+(.+)$/u;
const progressReport=/^(?:eu )?(?:(?:ainda )?nao (?:consegui|fiz|avancei|terminei|conclui)|avancei|enviei|revisei|escrevi|comecei|iniciei|testei|liguei|recebi|fiquei (?:travado|travada|bloqueado|bloqueada)|estou (?:travado|travada|bloqueado|bloqueada))\b/u;
...
export function trackingCommitmentCandidates(message:string):string[]{
 ...
 return clausesOf(message).filter(quote=>{
  const s=normalized(quote),match=s.match(concreteStep);
  return quote.length>=8&&quote.length<=500&&!s.endsWith("?")&&!/\b(?:se|caso|talvez|tentar|poderia|eventualmente)\b/u.test(s)&&!!match&&subjectWords(match[1]).size>0&&!/^(?:isso|isto|esse|essa|aquilo|algo|alguma coisa|o mesmo)\b/u.test(match[1]);
 });
}
...
export function commitmentActionTargets(quote:string,commitments:CoachCommitment[],type:string):CoachCommitment[]{
 ...
 if(!words.size)return type==="complete_commitment"&&candidates.length===1?candidates:[];
...
export function actionQuoteCandidates(message:string,type:string):string[]{
 ...
 return [...new Set(candidates)].filter(quote=>quote.length>=(type==="complete_commitment"?6:8)&&quote.length<=900&&allowedActions(quote).includes(type)&&![...message.matchAll(quotedText)].some(span=>{const at=message.indexOf(quote);return at>=span.index&&at<span.index+span[0].length;}));
}
...
export function validateAction(raw:unknown,message:string,commitments?:CoachCommitment[]):UserAction|null{
 ...
 if(typeof a.quote!=="string"||a.quote.length<(a.type==="complete_commitment"?6:8)||!message.includes(a.quote)||!allowedActions(message).includes(a.type)||!allowedActions(a.quote).includes(a.type))return null;
