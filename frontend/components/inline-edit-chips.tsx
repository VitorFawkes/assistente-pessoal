...
function InlinePopover({
  ...
  ariaLabel: string;
  width?: number;
  // Altura estimada do conteúdo — decide se abre pra cima quando falta espaço.
  minHeight?: number;
  onOpen?: () => void;
  ...
  function toggle(e: React.MouseEvent) {
    ...
    if (!open) {
      ...
      if (r) {
        ...
        const openUp = spaceBelow < minHeight && r.top > spaceBelow;
        setPos(
          openUp
...
// ─── Mini-calendário (próprio, sem o seletor nativo do SO) ─────────────
// ─── Prazo ────────────────────────────────────────────────────────────
    ...
    >
      {(close) => (
        <div className="flex flex-col">
          ...
          {tarefa.prazo && (
            <>
              <div className="mt-1 border-t border-[color:var(--border)]" />
      ...
      trigger={
        <>
          ...
          <span className="truncate">
            {atual ? areaLabel(atual) : proposta ? `${areaLabel(proposta)}?` : "+ área"}
          </span>
        </>
      }
    >
