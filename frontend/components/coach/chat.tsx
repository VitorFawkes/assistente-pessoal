...
export function ChatView({ messages, busy, enabled, available, draft, onDraft, mutate }: { messages: CoachMessage[]; busy: boolean; enabled: boolean; available: boolean; draft: string; onDraft: (value: string) => void; mutate: CoachMutation }) {
  ...
  useEffect(() => {
    if (!followLatest.current || !history.current) return;
    history.current.scrollTop = history.current.scrollHeight;
    if (!pending) followLatest.current = false;
  ...
  return (
    <div className="space-y-5">
      ...
      <form className="space-y-3" onSubmit={async (event) => { event.preventDefault(); const message = draft.trim(); if (!canSend || !message) return; followLatest.current = true; setPending(message); const ok = await mutate({ action: "chat", message }); setPending(""); if (ok) onDraft(""); }}>
