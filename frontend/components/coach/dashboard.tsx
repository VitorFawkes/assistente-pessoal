...
type ViewState=CoachState & {calendar?:CalendarStatus;jobs?:CoachJob[];model?:{provider:string;model:string;reviewer_model?:string|null;semantic_enabled?:boolean}};
...
export function CoachDashboard() {
  ...
  if (!state) return (
    <div className="space-y-6"><header><h1 className="font-display text-4xl">Seu coach pessoal</h1><p className="mt-3 text-sm text-muted-strong">Seus objetivos, dificuldades e próximos passos.</p></header>{loading ? <div className="flex min-h-48 items-center justify-center gap-2 text-sm text-muted-strong" role="status"><LoaderCircle size={18} className="animate-spin motion-reduce:animate-none" aria-hidden="true" /> Carregando seu contexto…</div> : <div className="rounded-xl border border-border bg-card p-5"><p role="alert" className="text-sm text-[var(--urgent)]">{error}</p><button type="button" className={`${buttonClass} mt-4`} onClick={() => { setLoading(true); setError(null); void load(); }}><RefreshCw size={15} aria-hidden="true" /> Tentar novamente</button></div>}</div>
  );

  const { profile, coverage } = state;
  const reportReady=coverage.report_ready_meetings??0;
  const hasPersonalContext=!!(profile.goals.trim()||profile.context.trim())
    ||state.memories.some(item=>item.status!=="rejected"&&!item.stale&&!!item.content.trim())
    ||(state.commitments??[]).some(item=>!!item.title.trim())
    ||state.messages.some(item=>!item.stale&&!!item.content.trim()&&(item.role==="user"||item.context_freshness==="current"));
  const hasReviewContext=hasPersonalContext||reportReady>0||coverage.analyzed_chunks>0;
  ...
  return (
    <div className="min-w-0 space-y-6 sm:space-y-8">
      <header className="space-y-3">
        ...
        <h1 className="font-display text-[2.25rem] sm:text-[2.75rem] leading-[1.1] tracking-tight">Seu coach pessoal</h1>
        <p className={`max-w-lg text-sm leading-relaxed text-muted-strong ${state.messages.length ? "hidden sm:block" : ""}`}>Um lugar para escolher prioridades, enfrentar dificuldades e retomar o que você combinou.</p>
      </header>
      ...
      {settings && <div id="coach-settings"><SettingsView profile={profile} calendar={state.calendar} busy={!!busy} mutate={mutate} onClose={() => setSettings(false)} /></div>}
      ...
      <div ref={panel} className="scroll-mt-20">
        ...
        <div id={`coach-panel-${tab}`} role="tabpanel" aria-labelledby={`coach-tab-${tab}`} className="min-w-0 pt-6 sm:pt-8" tabIndex={0}>
          {tab === "week" && <div className="space-y-7">
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs leading-relaxed text-muted-strong">{profile.weekly_enabled ? `Preferência semanal: ${days[profile.review_day]?.toLowerCase()}, às ${String(profile.review_hour).padStart(2, "0")}:00 · ${profile.timezone.replace("America/", "").replaceAll("_", " ")}` : "Revisão automática desativada"}{!profile.enabled ? " · Coach pausado" : ""}</p><button className={`${buttonClass} !text-xs`} type="button" disabled={!!busy || !operational || !hasReviewContext} onClick={() => void mutate({ action: "review" }, "Sua revisão está disponível.")}><RefreshCw size={13} aria-hidden="true" /> Preparar revisão</button></div>
            {current ? <ReviewView review={current} onDiscuss={discuss} /> : <section className="rounded-xl border border-dashed border-border px-5 py-7"><BookOpen size={23} className="mb-3 text-[var(--calm)]" aria-hidden="true" /><h2 className="font-display text-2xl">Reveja o que importa para você</h2><p className="mt-3 text-sm leading-relaxed text-muted-strong">{hasPersonalContext ? "Seu contexto já permite preparar uma revisão. Retome objetivos, dificuldades e combinados para escolher o próximo passo; o coach deve perguntar sobre resultados que ainda não conhece." : hasReviewContext ? "Os registros disponíveis já permitem preparar uma revisão. O coach pode ajudar a encontrar uma prioridade e perguntar o que ainda precisa entender sobre seus objetivos." : "Conte um objetivo, uma dificuldade ou um combinado na conversa, ou registre seu contexto nos Ajustes. A revisão começa pelo que você compartilha e pode acontecer mesmo sem reuniões."}</p>{coverage.total_meetings > 0 && !hasReviewContext && <button className={`${primaryClass} mt-5`} type="button" disabled={!!busy || !operational} onClick={() => void mutate({ action: "analyze" }, "Cobertura atualizada.")}>Completar contexto faltante <ArrowRight size={15} aria-hidden="true" /></button>}</section>}
      ...
      <section aria-label="Contexto das reuniões" className="border-t border-border pt-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-medium">Reuniões que acrescentam contexto</h2><p className="mt-1 text-xs leading-relaxed text-muted-strong">{reportReady} de {coverage.total_meetings} {coverage.total_meetings === 1 ? "reunião com relatório ou resumo disponível" : "reuniões com relatório ou resumo disponível"}</p></div><button type="button" className={`${buttonClass} !text-xs`} disabled={!!busy || !operational || coverage.total_meetings === 0} onClick={() => void mutate({ action: "analyze" }, "Cobertura atualizada.")}><RefreshCw size={13} aria-hidden="true" />{coverage.pending_meetings > 0 ? "Completar contexto faltante" : "Verificar novas reuniões"}</button></div>
        {coverage.total_meetings > 0 && <div role="progressbar" aria-label="Reuniões com relatório ou resumo disponível" aria-valuemin={0} aria-valuemax={coverage.total_meetings} aria-valuenow={Math.min(reportReady, coverage.total_meetings)} className="my-3 h-1.5 overflow-hidden rounded-full bg-accent"><div className="h-full rounded-full bg-[var(--calm)]" style={{ width: `${Math.min(100, (reportReady / coverage.total_meetings) * 100)}%` }} /></div>}
        <p className="mt-2 text-[11px] leading-relaxed text-muted-strong">{coverage.total_meetings > 0 ? `${coverage.executive_report_meetings??0} relatórios detalhados · ${coverage.summary_only_meetings??0} resumos curtos. Esses registros e as tarefas ajudam a entender seu contexto; a transcrição pode aprofundar ou sustentar uma observação.` : "Nenhuma reunião finalizada disponível para consulta."}</p>
        {coverage.pending_meetings>0&&<p className="mt-2 text-[11px] leading-relaxed text-muted-strong">{coverage.pending_meetings} reuniões sem relatório ainda precisam de leitura. Isso não impede conversar ou preparar uma revisão com o contexto já disponível.</p>}
        {coverage.analyzed_chunks>0&&<p className="mt-2 text-[11px] leading-relaxed text-muted-strong">Leituras aprofundadas já registradas: {coverage.analyzed_chunks} trechos de transcrição. Disponibilidade de relatório não significa avaliação completa de cada comportamento.</p>}
        ...
        {state.model&&<details className="mt-3 text-[11px] text-muted-strong"><summary className="min-h-9 cursor-pointer py-2">Modelo e contexto disponíveis</summary><p>Coach: {state.model.model} ({state.model.provider}).{state.model.reviewer_model?` Revisão adicional: ${state.model.reviewer_model}.`:""}</p><p className="mt-1">A disponibilidade depende do acesso configurado ao provedor. A agenda usa a conexão do TTARS quando disponível nesta conta; consulte o estado em Ajustes. Conversas dos agentes fora do Ações não estão conectadas.</p></details>}
