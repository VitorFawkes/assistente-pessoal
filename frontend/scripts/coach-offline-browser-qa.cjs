...
(async () => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Adicionar objetivo de trabalho", exact: true }).click();
    await page.getByRole("textbox", { name: "O que é" }).fill("Objetivo fictício de trabalho");
    await page.getByRole("button", { name: "Salvar", exact: true }).click();
    await page.getByText("Objetivo salvo.").waitFor();
    check(await page.evaluate(() => window.__fixture.state().goals.some(goal => goal.area === "work" && goal.content === "Objetivo fictício de trabalho")), "goal editor sends a work goal to synthetic API");
    ...
    check(await page.evaluate(() => window.__fixture.state().profile.context) === "Contexto ainda não salvo", "save sends preserved draft to synthetic API");
    check(await page.evaluate(() => !("goals" in window.__fixture.calls.filter(call => call.action === "settings").at(-1))), "saving settings never rewrites goals");
    await page.evaluate(() => window.__fixture.remount());
    await page.getByRole("button", { name: "Ajustes", exact: true }).click();
    check(await page.getByText("Objetivo fictício de trabalho").isVisible(), "remount lists the saved goal");
    check(await context.inputValue() === "Contexto ainda não salvo", "remount reads saved synthetic settings");
