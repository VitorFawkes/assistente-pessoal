...
// ─── modo paralelo ───────────────────────────────────────────────────
// --paralelo N: várias reuniões ao mesmo tempo. Aí a lista de cada reunião é
// tudo o que nasceu antes dela, SEM tirar o que o ensaio juntou (as reuniões não
// esperam umas pelas outras). Serve pra medir taxas e conferir os pares; a
// simulação exata é a sequencial, abaixo.
const paralelo = Number(arg("paralelo") ?? 0);
if (paralelo > 0) {
  const DIA_MS = 86_400_000;
  const fila = reunioes.filter((r) => !feitas.has(r.id) && (soReunioes.size === 0 || soReunioes.has(r.id)));
  let proximo = 0;
  const todas = [...validas];
  async function trabalhador() {
    while (proximo < fila.length) {
      const r = fila[proximo++];
      const T = r.em;
      const tMs = Date.parse(T);
      const candidatas: Candidata[] = [];
      const vetoresCandidatas = new Map<string, number[]>();
      for (const l of todas) {
        const antes = l.meeting_id ? (l.reuniao_em ?? l.created_at) < T : l.created_at <= T;
        if (!antes || l.meeting_id === r.id) continue;
        const concluidaAntes = l.concluida_em && l.concluida_em < T;
        if (concluidaAntes && Date.parse(l.concluida_em!) < tMs - DIAS_CONCLUIDA * DIA_MS) continue;
        candidatas.push({
          id: l.id,
          titulo: l.titulo,
      ...
      const registros: Registro[] = r.tarefas.map((t, i) => {
        ...
        return {
          ...
          similaridade: alvo ? res.mostradas[i].find((m) => m.id === alvo.id)?.similaridade ?? null : null,
      ...
      appendFileSync(arqUso, JSON.stringify({ reuniao: r.id, candidatas: candidatas.length, ms: Date.now() - inicio, ...res.uso }) + "\n");
