...
A validação da conversa real motivou uma correção: o servidor agora preserva na mensagem os campos de observação, hipótese e outra explicação já fundamentados pelo modelo, em vez de exibir somente sua prosa livre. O contexto consultado e a data da contagem de cobertura também ficam explícitos. A correção passou em 214 testes unitários, build e um ciclo real com o provedor usando somente dados fictícios (26 verificações). A revisão semanal deixa de gravar contagens globais transitórias do acervo.
