...
- `OPENAI_API_KEY`: somente no servidor. `COACH_SEMANTIC_ENABLED=true` habilita embeddings `text-embedding-3-small`; o uso de cada chamada (tokens e custo, sem prompts/respostas) é sempre gravado em `coach_model_runs`, porque é a base do teto diário `COACH_DAILY_BUDGET_USD` (padrão US$ 3); `COACH_AUDIT_ENABLED` não controla mais essa gravação.
...
Esta correção passou pelos testes unitários e por 48 integrações com Postgres isolado, incluindo persistência, concorrência e isolamento. As migrations 0032–0034 foram aplicadas em produção após backup privado validado, com conferência de colunas, constraints, grants e RLS. O estado da publicação do frontend está registrado em coach-deployment.md. O teste de navegador isolado `frontend/scripts/coach-offline-browser-qa.cjs` usa os componentes reais e uma API em memória: não prova autenticação nem persistência real.
...
- Acordos concretos e explícitos, como “Vou enviar a proposta”, usam `track_commitment`, sem tarefa ou evento. Aceites vagos não autorizam uma alteração ambígua. O prazo não é inventado: datas estruturadas exigem ISO literal; um acordo sem data continua disponível para retomada na conversa e no fechamento. Renegociar sem nova data exata limpa o prazo antigo do acordo e da tarefa vinculada, preservando o relato literal no histórico.
...
Validação de 21/09: 396 testes unitários, 48 integrações Postgres com duas contas, TypeScript/lint, 24 verificações de navegador desktop/mobile e CI com build de produção passaram. Sol real passou em 17 cenários pessoais, quatro de agenda e dois ciclos de persistência com dados sintéticos. O harness usa a mesma verificação e uma única tentativa de reparo do serviço; respostas e evidências foram inspecionadas em arquivos privados. A publicação e a prova autenticada em produção estão registradas em coach-deployment.md. Testes desativados não contam como prova de integração.
